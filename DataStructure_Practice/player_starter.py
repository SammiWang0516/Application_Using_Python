...
class Player:
    ...
    def addMedia(self, media) -> None:
        ...
        if currentMediaNode is None:
            currentMediaNode = self.playlist.dummyHead.next
    ...
    def loadFromJson(self, fileName) -> None:
        ...
        for dict in data:
            ...
            if dict.get('wrapperType') == 'track' and dict.get('kind') == 'feature-movie':
                if dict.get('trackTimeMillis'):
                    movie_duration = round(dict.get('trackTimeMillis') / 1000 / 60)     # milisecond to min
                else:
                    movie_duration = None
                movie = Movie(dict.get('trackName'),                                    # movie title 
                              dict.get('artistName'),                                   # movie artist
                              dict.get('releaseDate')[:10],                             # movie release date
                              dict.get('previewUrl'),                                   # movie url  
                              dict.get('contentAdvisoryRating'),                        # movie rating
                              movie_duration)                                           # movie duration
                self.playlist.append(movie)

            # if such dictionary represent track
            elif dict.get('wrapperType') == 'track':
                if dict.get('trackTimeMillis'):
                    track_duration = round(dict.get('trackTimeMillis') / 1000)          # milisecond to sec
                else:
                    track_duration = None
                track = Track(dict.get('trackName'),                                    # track title 
                              dict.get('artistName'),                                   # track artist 
                              dict.get('releaseDate')[:10],                             # track release date
                              dict.get('previewUrl') or dict.get('trackViewUrl'),       # track url 
                              dict.get('collectionName'),                               # track album 
                              dict.get('genres') or dict.get('primaryGenreName'),       # track genre
                              track_duration)                                           # track duration
                self.playlist.append(track)
            
            # others would be media
            else:
                media = Media(dict.get('collectionName'),                               # media title
                              dict.get('artistName'),                                   # media artist
                              dict.get('releaseDate')[:10],                             # media release date
                              dict.get('previewUrl'))                                   # media url
                self.playlist.append(media)
